    ...
    while (udpSocket->hasPendingDatagrams()) {
        ...
        if (message.startsWith("SatoriEye_DISCOVERY_RESPONSE")) {
            ...
            if (messageParts.size() > 1) {
                QString batteryLevel = messageParts[1]; // 电量信息在第二部分
                int batteryPercentage = batteryLevel.toInt();
                emit batteryInfoReceived(batteryPercentage); // 发出电量信号
            }

            setMode(MobileClient::EyeMode::Auto);
            heartbeatTimeout = 0; // 重置心跳超时计数
        } else if (message.startsWith("SatoriEye_HEARTBEAT_RESPONSE")) {
            heartbeatTimeout = 0; // 重置超时计数
            // 这里可以省略电量信息处理
